...
class ImportWorkflowsCommand extends base_command_1.BaseCommand {
    ...
    async getWorkflowOwner(workflowId) {
        ...
        if (sharing && sharing.project.type === 'personal') {
            const user = await di_1.Container.get(user_repository_1.UserRepository).findOneByOrFail({
                projectRelations: {
    ...
    async workflowExists(workflowId) {
        return await di_1.Container.get(workflow_repository_1.WorkflowRepository).existsBy({ id: workflowId });
    }
