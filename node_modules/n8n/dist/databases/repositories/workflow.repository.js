...
let WorkflowRepository = class WorkflowRepository extends typeorm_1.Repository {
    constructor(dataSource, globalConfig, folderRepository) {
        super(db_1.WorkflowEntity, dataSource.manager);
    ...
    applyTagsFilter(qb, filter) {
        if ((0, db_1.isStringArray)(filter?.tags) && filter.tags.length > 0) {
            const subQuery = qb
                .subQuery()
                .select('wt.workflowId')
                .from(db_1.WorkflowTagMapping, 'wt')
                .innerJoin(db_1.TagEntity, 'filter_tags', 'filter_tags.id = wt.tagId')
    ...
    async findWebhookBasedActiveWorkflows() {
        return await this.createQueryBuilder('workflow')
            ...
            .innerJoin(db_1.WebhookEntity, 'webhook_entity', 'workflow.id = webhook_entity.workflowId')
            .execute();
    }
    ...
    async moveAllToFolder(fromFolderId, toFolderId, tx) {
        await tx.update(db_1.WorkflowEntity, { parentFolder: { id: fromFolderId } }, {
